...
import { Search, X, FolderOpen, Loader as Loader2, Heart, Trash2, Menu, Music as MusicIcon, TrendingUp } from 'lucide-react';
      ...
      <div className="w-80 rounded-2xl p-6 shadow-2xl animate-slide-up"
        ...
        <div className="flex gap-2">
          ...
          <button onClick={() => { if (name.trim()) { onCreated(name.trim()); onClose(); } }} disabled={!name.trim()}
            className="flex-1 py-2.5 rounded-xl text-black font-semibold text-sm transition-all hover:opacity-90 disabled:opacity-40"
...
export default function App() {
  ...
  // already stored for every song that has no art, using the same
  // extractMeta() import uses. Covers songs imported before an art-parsing
  // bug fix landed (see extractMeta's comments) -- those files never get a
  // second look otherwise, since importing only runs once per file.
  ...
  const handleRescanArt = useCallback(async () => {
    ...
    showToast(result.fixed > 0
      ? `Found art for ${result.fixed} of ${result.scanned} song${result.scanned === 1 ? '' : 's'}`
      : result.scanned > 0 ? 'No missing art could be recovered' : 'No songs are missing art');
  }, [songs]);
