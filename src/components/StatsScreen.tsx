...
import { TrendingUp, BarChart3, Clock, Music2, Disc3, User, Play, Trash2 } from 'lucide-react';
...
export function StatsScreen({ songs, history, accentColor, onClearHistory, onPlaySong, listeningStats }: Props) {
  const stats = useMemo(() => {
    ...
    return { totalPlays, top10, maxCount, topArtist, topArtistCount, topAlbum, topAlbumCount };
  }, [songs]);
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-6">
      ...
      <div className="flex-1 flex flex-col items-center justify-center text-white/25 gap-2 py-20">
        ...
        <p className="font-medium text-white/40">No listening stats yet</p>
        <p className="text-xs">Play some music to see your stats here</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex-1 overflow-y-auto pb-6">
      <ListeningStats stats={listeningStats} accentColor={accentColor} />
      {/* Summary cards */}
      <div className="grid grid-cols-2 gap-3 px-4 pt-3 mb-4">
        ...
        <StatCard icon={<User size={18} />} label="Top Artist" value={stats.topArtist || '—'} sub={stats.topArtist ? `${stats.topArtistCount} plays` : undefined} accentColor={accentColor} />
      ...
      <div className="px-4 mb-6">
        ...
        <div className="space-y-2">
          {stats.top10.map((song, i) => {
            ...
            return (
              <div key={song.id} className="flex items-center gap-3 cursor-pointer group" onClick={() => onPlaySong(song)}>
                <span className="text-white/30 text-xs font-bold w-5 text-right shrink-0">{i + 1}</span>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="text-xs font-bold tabular-nums shrink-0" style={{ color: accentColor }}>{song.playCount}</span>
                  </div>
        ...
        {history.length === 0 ? (
          ...
          <div className="space-y-0.5">
            {history.slice(0, 30).map((entry) => {
              ...
              if (!song) return null;
              return (
                <div key={entry.id} className="flex items-center gap-3 py-1.5 group cursor-pointer" onClick={() => onPlaySong(song)}>
