        ...
        <div className="flex items-center gap-2">
          ...
          >
            <div className="w-full h-[3px] rounded-full bg-white/15">
              <div className="h-full rounded-full" style={{ width: `${progress}%`, background: 'rgba(255,255,255,0.6)' }} />
            </div>
          </div>
      ...
      <div className="hidden md:flex relative h-full items-center px-4 gap-3">
        ...
        <div className="flex items-center gap-2 w-[28%] justify-end shrink-0">
          <button onClick={onOpenQueue} className="btn-icon w-8 h-8 hover:bg-white/10 rounded-lg relative" title="Queue">
            ...
            {queueCount > 0 && (
              <span className="absolute -top-1 -right-1 text-[9px] font-bold w-4 h-4 flex items-center justify-center rounded-full text-black"
                style={{ background: accentColor }}>{queueCount > 9 ? '9+' : queueCount}</span>
            )}
          </button>
