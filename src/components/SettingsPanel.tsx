...
interface Props {
  ...
  /** Re-scans every song missing album art against its already-stored audio
   *  blob (see scanner.ts's rescanMissingArt for why this is needed). */
  ...
  return (
    <div ref={overlayRef} className="fixed inset-0 z-50 flex items-center justify-center px-4"
      ...
      <div className="w-full max-w-sm max-h-[85vh] overflow-y-auto rounded-2xl p-6 shadow-2xl animate-slide-up"
        ...
        <button onClick={onClose} className="w-full mt-5 py-3 rounded-xl font-semibold text-black transition-all hover:opacity-90 active:scale-[0.98]"
          style={{ background: accentColor }}>Done</button>
        ...
        <div className="mt-5 pt-4 border-t border-white/10">
          ...
          <p className="text-white/30 text-xs mt-2 leading-snug">
            Re-checks already-imported songs for embedded cover art that a
            past bug may have missed. Songs whose files never had art won't
            be affected.
          </p>
        </div>
