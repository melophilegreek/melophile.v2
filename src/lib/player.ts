...
class Player {
  ...
  // Called when a song starts playing — used to record play count
  onPlayStart: ((song: Song) => void) | null = null;

  constructor() {
    const a = this.audio;
    a.volume = 0.8;
    a.addEventListener('timeupdate', () => this._patch({ currentTime: a.currentTime }));
  ...
  private _handleEnd() {
    ...
    if (repeat === 'one') { this.seek(0); this.play(); return; }
    this.next(true);
  }
