...
/** Re-parses embedded artwork for every already-imported song that doesn't
 *  have art, using the audio blob already sitting in IndexedDB -- no need to
 *  re-select the original folder/files. This exists because art extraction
 *  has had several bug fixes over time (large ID3/FLAC/M4A tags pushing the
 *  cover art past the head-chunk read -- see extractMeta's comments above);
 *  those fixes only apply to *new* imports, so a song imported before a fix
 *  landed keeps showing a placeholder forever unless it's re-scanned like
 *  this. Folder-level cover.jpg/folder.jpg fallback art can't be retried
 *  here, since only the audio file itself (not its sibling files) is
 *  stored -- some songs may still come up empty if they never had embedded
 *  art to begin with. */
export async function rescanMissingArt(
  onProgress: (p: ArtRescanProgress) => void,
): Promise<{ scanned: number; fixed: number }> {
  const songs = (await getAllSongs()).filter((s) => !s.albumArtData);
